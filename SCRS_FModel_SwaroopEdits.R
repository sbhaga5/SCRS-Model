...
FileName <- 'SCRS_Model_Inputs_2021.xlsx'
...
for(i in StartIndex:length(FYE)){
  ...
  BlendedDR[i] <- ((1 + NewDR_CurrentHires[i]) / (1 + AmoBaseInc_CurrentHires)) - 1
  ...
  AALOrigDR_CurrentHires[i] <- AALOrigDR_CurrentHires[i-1]*(1+OriginalDR_CurrentHires[i]) + (MOYNCExistOrigDR[i] + BenPayments_CurrentHires[i] + Refunds_CurrentHires[i])*(1+OriginalDR_CurrentHires[i])^0.5
  AALOrigDR_NewHires[i] <- AALOrigDR_NewHires[i-1]*(1+OriginalDR_NewHires[i]) + (MOYNCNewHiresOrigDR[i] + BenPayments_NewHires[i] + Refunds_NewHires[i])*(1+OriginalDR_NewHires[i])^0.5
  AALOrigDR_Total[i] <- AALOrigDR_CurrentHires[i] + AALOrigDR_NewHires[i]
  ...
  AALNewDR_CurrentHires[i] <- AALOrigDR_CurrentHires[i]*((1+(LiabSensDR/100))^(DRDifference_NewHires))*((1+(Convexity/100))^((DRDifference_NewHires)^2/2))
  AALNewDR_NewHires[i] <- AALOrigDR_NewHires[i]*((1+(LiabSensDR/100))^(DRDifference_NewHires))*((1+(Convexity/100))^((DRDifference_NewHires)^2/2))
  AALNewDR_Total[i] <- AALOrigDR_Total[i]*((1+(LiabSensDR/100))^(DRDifference_NewHires))*((1+(Convexity/100))^((DRDifference_NewHires)^2/2))
}
...
RunModel <- function(Analysis_Type = AnalysisType, 
  ...
  if(ERPolicyCurrentHires == 'Statutory Rate'){
    ...
    AmoFundPeriod_Max <- AmoPeriod2028
  }
  ...
  AmoFundPeriod_Min <- AmoPeriod2028
  ...
  for(i in 1:nrow(OffsetYears_CurrentHires)){
    for(j in 1:i){
      AmoDecrement <- max(AmoFundPeriod_Max - 4 - i, AmoFundPeriod_Min)
      OffsetYears_CurrentHires[i,j] <- max(AmoDecrement - j + 2, 1) 
    }
  }
  ...
  for(i in 1:nrow(OffsetYears_NewHires)){
    for(j in 1:i){
      AmoDecrement <- max(AmoFundPeriod_Max - 4 - i, AmoFundPeriod_Min)
      OffsetYears_NewHires[i,j] <- max(AmoDecrement - j + 2, 1) 
    }
  }
  ...
  for(i in StartIndex:length(FYE)){
    ...
    Solv_Contrib_CurrentHires[i] <- Solv_Contrib_Total[i]*(AALNewDR_CurrentHires[i]/AALOrigDR_Total[i])
    Solv_Contrib_NewHires[i] <- Solv_Contrib_Total[i]*(AALNewDR_NewHires[i]/AALOrigDR_Total[i])
    ...
    NetCF_CurrentHires[i] <- BenPayments_CurrentHires[i] + Refunds_CurrentHires[i] +  AdminExp_CurrentHires[i] +
      ...
      ER_Amo_ORP_CurrentHires[i] + ER_Amo_RehRet[i] + ER_Amo_Stat_NewHires[i] + ER_Amo_ORP_NewHires[i] + 
      Solv_Contrib_CurrentHires[i] + StatContFloor[i]
    ...
    NetCF_NewHires[i] <- BenPayments_NewHires[i] + Refunds_NewHires[i] + AdminExp_NewHires[i] + EE_NC_NewHires[i] +
      EE_Amo_NewHires[i] + ER_NC_NewHires[i] + ER_Amo_ADC_NewHires[i] + Solv_Contrib_NewHires[i]
    ...
    UAL_AVA_CurrentHires[i] <- AALNewDR_CurrentHires[i] - AVA_CurrentHires[i]
    UAL_MVA_CurrentHires[i] <- AALNewDR_CurrentHires[i] - MVA_CurrentHires[i]
    
    AVA_NewHires[i] <- MVA_NewHires[i] - TotalDefered_NewHires[i]
    UAL_AVA_NewHires[i] <- AALNewDR_NewHires[i] - AVA_NewHires[i]
    UAL_MVA_NewHires[i] <- AALNewDR_NewHires[i] - MVA_NewHires[i]
    ...
    FR_AVA[i] <- AVA[i] / AALNewDR_Total[i]
    FR_MVA[i] <- MVA[i] / AALNewDR_Total[i]
    ...
    if(ProjectionCount < nrow(Amortization_CurrentHires)){
      #Oustanding Balance
      if(FR_AVA[i] >= 1 && FR_AVA[i-1] < 1){
        OutstandingBase_CurrentHires[ProjectionCount+1,2:(ProjectionCount + 1)] <- 0
      } else {
      ...
      Amortization_CurrentHires[ProjectionCount+1,2:(ProjectionCount + 1)] <- PMT(pv = OutstandingBase_CurrentHires[ProjectionCount+1,2:(ProjectionCount + 1)], 
                                                                                  r = NewDR_CurrentHires[i], g = AmoBaseIncCurrentHires, t = 0.5,
    ...
    if(ProjectionCount < nrow(Amortization_NewHires)){
      #Oustanding Balance
      if(FR_AVA[i] >= 1 && FR_AVA[i-1] < 1){
        OutstandingBase_NewHires[ProjectionCount+1,2:(ProjectionCount + 1)] <- 0
      } else {
      ...
      Amortization_NewHires[ProjectionCount+1,2:(ProjectionCount + 1)] <- PMT(pv = OutstandingBase_NewHires[ProjectionCount+1,2:(ProjectionCount + 1)],
                                                                              r = NewDR_NewHires[i], g = AmoBaseIncNewHires, t = 0.5,
